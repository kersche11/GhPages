  ...
  constructor(scene: Phaser.Scene, x: number, y: number, texture: string) {
    ...
    this.on('dragend', () => {
      ...
      if (this.isOverSquare && draggedNumberBeforeCheck !== null) {
        ...
        if (
          parseInt(<string>draggedNumberBeforeCheck ) +
          parseInt(OperationController.instance.secondNumber) ===
          parseInt(OperationController.instance.result)
        )
        {
          ...
          if(OperationController.instance.correctAnswersCount < 5){
            ...
            //OperationController.instance.animation();
            setTimeout(() => {
          ...
          }, 1000); // Verzögerung von 3000 Millisekunden (3 Sekunden)
        }
      }
      else {
        // Animation if not dropped over the empty square
...
// Zeichne eine schwarze Umrandung
    ...
    popupText.on('pointerdown', () => {
      const nochmal_sound = this.scene.sound.add('nochmalspielen');
      nochmal_sound.play();
    });
    ...
    daumenrunter.on('pointerdown', () => {
      ...
      popupContainer.destroy(); // Popup-Fenster schließen
      this.scene3.navigateToSceneComponent();
